...
type Client struct {
	...
	healthCheckCallback		  func(map[string]bool)
...
func NewClient(options ...ClientOptionFunc) (*Client, error) {
	...
	if c.healthcheckEnabled {
		...
		c.healthcheck(c.healthcheckTimeoutStartup, true, nil)
	}
...
func SetHealthcheckCallback(f func(map[string]bool)) ClientOptionFunc {
	return func(c *Client) error {
		c.healthCheckCallback = f
		return nil
	}
}
...
func (c *Client) healthchecker() {
	...
	for {
		...
		case <-c.healthcheckStop:
			...
			return
		case <-ticker.C:
			c.healthcheck(timeout, false, c.healthCheckCallback)
		}
	}
}
...
func (c *Client) healthcheck(timeout time.Duration, force bool, callback func(map[string]bool)) {
	c.mu.RLock()
	...
	for {
		...
		if errors.Cause(err) == ErrNoClient {
			...
			if !retried {
				...
				c.healthcheck(timeout, false, nil)
			}
