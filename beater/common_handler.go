...
func logHandler(h http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			internal.SendCnt(w, r, server.InternalError(err))
		}
...
func killSwitchHandler(killSwitch bool, h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if killSwitch {
			...
			internal.SendCnt(w, r, server.Forbidden(errors.New("endpoint is disabled")))
		}
	})
}
...
func corsHandler(allowedOrigins []string, h http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "OPTIONS" {
			...
			internal.Send(w, r, server.EmptyResponse(http.StatusOK))
			...
			internal.SendCnt(w, r, server.Forbidden(errors.New("origin: '"+origin+"' is not allowed")))
		}
	})
}
