...
func agentConfigHandler(kbClient *kibana.Client, secretToken string) http.Handler {
	...
	var handler http.Handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case requestErr != nil:
			send(server.BadRequest(requestErr))
		case query == agentcfg.Query{}:
			send(server.MethodNotAllowed())
		case internalErr != nil:
			send(server.InternalError(internalErr))
		case len(cfg) == 0:
			send(server.EmptyResponse(http.StatusNotFound))
		case clientEtag != "" && clientEtag == upstreamEtag:
			w.Header().Set("Cache-Control", "max-age=0")
			send(server.EmptyResponse(http.StatusNotModified))
		case upstreamEtag != "":
			w.Header().Set("Cache-Control", "max-age=0")
