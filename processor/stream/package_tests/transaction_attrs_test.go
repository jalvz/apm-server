...
func transactionProcSetup() *tests.ProcessorSetup {
	path := "../testdata/intake-v2/transactions.ndjson"
	payload, _ := LoadPayload(path)
	return &tests.ProcessorSetup{
		Decoder:         stream.DecoderFunc((&transformer.Transformer{}).DecodeTransaction),
		FullPayloadPath: path,
		SamplePayload:   payload.([]map[string]interface{})[1]["transaction"],
